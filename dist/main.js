...
function removeOldTeamData() {
    ...
    const rows = document.querySelectorAll(".fromAPIteam");
...
function fetchNewTeamData() {
    ...
    _axiosDefault.default.request({
        ...
        if (teamSearch == "") for(let k = 0; k < 30; k++){
            const u = ru.data[k];
        ...
        // photoCell.appendChild(image);
            ...
            const photoCell = document.createElement("td");
            aRow.appendChild(photoCell);
            photoCell.innerText = searchedTeam.conference;
        }
    });
}
function fetchNewPlayer() {
    const randomPlayer = Math.floor(Math.random() * 26);
    _axiosDefault.default.request({
        method: "GET",
        ...
        const aPlayer = ru.data[randomPlayer];
...
},{}]},["lpcHr","jeorp"], "jeorp", "parcelRequirefd0b")
